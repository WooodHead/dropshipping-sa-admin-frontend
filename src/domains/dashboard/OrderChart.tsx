...
const aggregateOrdersByDay = (
  ...
  orders
    .filter((order: GuestOrder) => order.status !== "cancelled")
    .reduce((acc, curr) => {
      const day = format(curr.createdAt, "YYYY-MM-DD")
      if (!acc[day]) {
        acc[day] = 0
      }
      acc[day] += curr.totalCost
      return acc
    }, {} as { [key: string]: number })

const getRevenuePerDay = (orders: GuestOrder[]): TotalByDay[] => {
...
const OrderChart: FC<{ orders?: GuestOrder[] }> = ({ orders }) => {
